...
async function getAllCities() {
    ...
    return db.collection("cinemaCatalog").find({}).project().toArray();
}
...
async function getMoviesByCinemaId(cinemaId) {
    ...
    return db.collection("cinemaCatalog").aggregate([
        ...
        {$unwind: "$cinemas"}, // faz o campo multivalorado se tornar univalorado
...
async function getMovieSessionsByCinemaId(movieId, cinemaId) {
    ...
    return sessions.map(item => item._id) //resolve o _id desnecessário
}
